...
from hytek_parser.hy3.schemas import ParsedHytekFile, DisqualificationInfo
...
def h1_parser(
    ...
    if entry.finals_dq_info:
        ...
        assert (
            entry.finals_dq_info[dq_code.value].code == dq_code
        ), "DQ Codes should match in the H1 line"

        entry.finals_dq_info[dq_code.value] = DisqualificationInfo(dq_code, dq_info)
    elif entry.swimoff_dq_info:
        # DQ happened in prelims
        assert (
            entry.swimoff_dq_info[dq_code].code == dq_code
        ), "DQ Codes should match in the H1 line"

        entry.swimoff_dq_info[dq_code.value] = DisqualificationInfo(dq_code, dq_info)
    elif entry.prelim_dq_info:
        # DQ happened in prelims
        assert (
            entry.prelim_dq_info[dq_code].code == dq_code
        ), "DQ Codes should match in the H1 line"

        entry.prelim_dq_info[dq_code.value] = DisqualificationInfo(dq_code, dq_info)
...
def h2_parser(
    ...
    assert (
        entry.prelim_dq_info or entry.swimoff_dq_info or entry.finals_dq_info
    ), "There must be a DQ for there to be an H2 line"
